...
static void check_initialization_complete(CustomData *data) {
    ...
    if (!data->initialized && data->native_window && data->main_loop) {
        GST_DEBUG ("Initialization complete, notifying application. native_window:%p main_loop:%p",
...
static void *app_function(void *userdata) {
    ...
    g_object_set(data->pipeline, "latency", (guint64) 500000000, NULL);
...
static void gst_native_init(JNIEnv *env, jobject thiz) {
    ...
    GST_DEBUG_CATEGORY_INIT (debug_category, "player", 0, "Babyphone Video Player");
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    JNIEnv *env = NULL;
    __android_log_print(ANDROID_LOG_INFO, "player", "Initializing jni file");
    java_vm = vm;
    ...
    jclass klass = (*env)->FindClass(env, "babyphone/frosi/babyphone/Player");
